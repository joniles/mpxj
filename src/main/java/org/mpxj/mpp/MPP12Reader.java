...
final class MPP12Reader implements MPPVariantReader
{
   ...
   private void processTaskData() throws IOException
   {
      ...
      for (Integer uniqueID : uniqueIdArray)
      {
         ...
         if (task.getName() == null && ((task.getStart() == null || task.getStart().equals(MicrosoftProjectConstants.EPOCH_DATE)) || (task.getFinish() == null || task.getFinish().equals(MicrosoftProjectConstants.EPOCH_DATE)) /*|| (task.getCreateDate() == null || task.getCreateDate().equals(MPPUtility.getEpochDate())*//* Valid tasks can have a null create date */))
         {
