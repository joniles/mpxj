...
public final class MSPDIReader extends AbstractProjectStreamReader implements HasCharset
{
   ...
   private List<TimephasedWork> readTimephasedWork(ProjectCalendar calendar, Project.Assignments.Assignment assignment, int type)
   {
      ...
      // Synchro also seems to produce overlapping items, which we also fix here.
      return fixOverlappingTimephasedItems(assignment.getTimephasedData().stream().filter(i -> NumberHelper.getInt(i.getType()) == type && i.getStart() != null && i.getFinish() != null).map(i -> readTimephasedWork(calendar, i)).filter(Objects::nonNull).collect(Collectors.toList()));
   }
   ...
   private List<TimephasedCost> readTimephasedCost(Project.Assignments.Assignment assignment, int type)
   {
      ...
      // Synchro also seems to produce overlapping items, which we also fix here.
      return fixOverlappingTimephasedItems(assignment.getTimephasedData().stream().filter(i -> NumberHelper.getInt(i.getType()) == type && i.getStart() != null && i.getFinish() != null).map(this::readTimephasedCost).collect(Collectors.toList()));
   }

   private TimephasedWork readTimephasedWork(ProjectCalendar calendar, TimephasedDataType item)
   {
      ...
      {
         workPerHour = Duration.getInstance(0, TimeUnit.MINUTES);
      }
      else
      {
         double workHours = calendar.getWork(item.getStart(), item.getFinish(), TimeUnit.HOURS).getDuration();
         if (workHours == 0.0)
         {
      ...
      // where there is zero work on the calendar. This aligns better
      // with MPP files and simplifies segmentation logic.
      if (workPerHour.getDuration() == 0.0 && calendar.getWork(item.getStart(), item.getFinish(), TimeUnit.MINUTES).getDuration() == 0)
      {
         return null;
      }
      ...
      timephasedWork.setAmountPerHour(workPerHour);
   ...
   /**
    * Synchro appears to occasionally write overlapping timephased items.
    * This method adjusts the start date of an item so that it doesn't overlap
    * with the end date of a previous item.
    *
   ...
   private <T extends TimephasedItem<?>> List<T> fixOverlappingTimephasedItems(List<T> items)
   {
      ...
      for (T item : items)
      {
         ...
         {
            item.setStart(previousItem.getFinish());
         }
         previousItem = item;
      }
      return items;
