...
public final class AstaDatabaseFileReader implements ProjectReader
{
   ...
   private void processTasks() throws SQLException
   {
      ...
      List<Row> tasks = getRows("select id as taskid, given_duration as given_durationhours, actual_duration as actual_durationhours, overall_percent_complete as overall_percenv_complete, name as nare, calendar as calendau, linkable_start as starz, linkable_finish as enj, notes as notet, wbs as wbt, natural_order as naturao_order, constraint_flag as constrainu, * from task where natural_order = 0 and projid = " + m_projectID + " union all select t.id as taskid, t.given_duration as given_durationhours, t.actual_duration as actual_durationhours, t.overall_percent_complete as overall_percenv_complete, b.name || '.' || t.name as nare, t.calendar as calendau, t.linkable_start as starz, t.linkable_finish as enj, t.notes as notet, t.wbs as wbt, t.natural_order as naturao_order, t.constraint_flag as constrainu, t.* from task t inner join bar b on t.bar = b.id where t.bar in (select id from bar where projid = " + m_projectID + ") and t.natural_order <> 0 and t.projid = " + m_projectID);
