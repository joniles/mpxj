...
public final class ResourceAssignment extends ProjectEntity implements ProjectEntityWithUniqueID, FieldContainer
{
   ...
   private CostRateTableEntry getCostRateTableEntry(Date date)
   {
      ...
      if (table == null)
      {
         ...
         result = new CostRateTableEntry(resource.getStandardRate(), resource.getOvertimeRate(), resource.getCostPerUse(), DateHelper.START_DATE_NA, DateHelper.END_DATE_NA);
      }
      else
      {
         if (table.size() == 1)
